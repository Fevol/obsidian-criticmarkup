...
function setSel(state: EditorState, selection: EditorSelection | {anchor: number, head?: number}, forward?: boolean, group?: boolean, select?: boolean) {
	let annotations: any = [];
...
function moveSel({state, dispatch}: CommandTarget, how: (range: SelectionRange) => SelectionRange, forward?: boolean, group?: boolean): boolean {
	const selection = updateSel(state.selection, how)
	if (selection.eq(state.selection)) return false
	dispatch(setSel(state, selection, forward, group))
	return true
}
...
function selectByLine(view: EditorView, forward: boolean) {
	return extendSel(view, range => view.moveVertically(range, forward), forward)
}
...
function cursorByPage(view: EditorView, forward: boolean) {
	...
	view.dispatch(setSel(state, selection, forward), {effects: effect})
	return true
}

function selectByPage(view: EditorView, forward: boolean) {
	return extendSel(view, range => view.moveVertically(range, forward, pageInfo(view).height), forward)
}
...
export const selectDocStart: StateCommand = ({state, dispatch}) => {
	dispatch(setSel(state, {anchor: state.selection.main.anchor, head: 0}, false))
	return true
}

export const selectDocEnd: StateCommand = ({state, dispatch}) => {
	dispatch(setSel(state, {anchor: state.selection.main.anchor, head: state.doc.length}, true))
	return true
}

export const cursorDocStart: StateCommand = ({state, dispatch}) => {
	dispatch(setSel(state, {anchor: 0}, true))
	return true
}

export const cursorDocEnd: StateCommand = ({state, dispatch}) => {
	dispatch(setSel(state, {anchor: state.doc.length}, false))
	return true
}
